...
export default function LaptopListingPage() {
  ...
  const filteredLaptops = useMemo(() => {
    ...
    if (minP !== undefined) {
      results = results.filter((laptop) => parseFloat(laptop.price) >= minP);
    }
    if (maxP !== undefined) {
      results = results.filter((laptop) => parseFloat(laptop.price) <= maxP);
    }
  ...
  const paginatedLaptops = filteredLaptops.slice(startIndex, startIndex + ITEMS_PER_PAGE);
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <h1 className="text-2xl font-bold">Skelbiu laptopai</h1>
            </div>
            {/* Search */}
            <div className="flex items-center space-x-4">
              <Input
            ...
            <Card key={laptop.title} className="flex flex-col">
              ...
              <CardFooter className="p-6 pt-0">
                <Button
                  className="w-full"
                  onClick={() =>
                    window.open(laptop.link, "_blank", "noopener,noreferrer")
                  }
                >
                  View on Skelbiu
                </Button>
